...
import com.avendum.midsautomate.selenium.seleniumpages.Dashboard;
import com.avendum.midsautomate.selenium.seleniumpages.DeployDashboard;
...
public class BasicTest {
    ...
    public boolean isOtpLoginWork(String otp){
        try{
            ...
            logger.severe("Login failed for user otp: " + otp + ". Error: " + e.getMessage());
            return false;
        }
    }
...
//                check=deployDashboard.launchCircleWisePage(driver);
    ...
    public File findDownloadedFile(String downloadDir, long startTime, String fileExtension, long timeout) throws InterruptedException {
...
//                sampleData.put(reportName, new String[]{"Success",String.valueOf(tookTime), downloadedFile.getAbsolutePath()});
                ...
                logger.info("No file downloaded within the timeout period.");
                return new String[]{"Failed", String.valueOf(tookTime), "No file downloaded"};
            }
        } catch (Exception e) {
            logger.info("An error occurred while downloading the report: " + e.getMessage());
